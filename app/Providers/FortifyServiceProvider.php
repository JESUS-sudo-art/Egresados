...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    {
        // Custom login response to redirect by role after authentication
                ...
                public function toResponse($request)
                {
                    ...
                    if ($user && method_exists($user, 'hasRole')) {
                        if ($user->hasRole('Administrador general')) {
                            return redirect()->intended(route('admin-general'));
                        }
                        if ($user->hasRole('Administrador de unidad')) {
                            return redirect()->intended(route('admin-unidad'));
                        }
    ...
    public function boot(): void
    {
        ...
        // Use Fortify to authenticate using the web guard with email/password
        Fortify::authenticateUsing(function (Request $request) {
            $userModel = app(config('auth.providers.users.model'));
            $user = $userModel::where('email', $request->input(Fortify::username()))->first();
